...
function findScript(project, eventType, branch) {
    ...
    return project.scripts.find((s) => s.event === eventType && s.branch === branch) || null;
}
...
export function createWebhookHandler({ config, logger, runner }) {
    return (req, res) => {
        ...
        // Keep service bootable without secret, but block webhook execution
        // until user configures server.secret from the control UI.
        if (!config.server.secret) {
            logger.warn('request rejected because server.secret is not configured', { ip });
            return res.status(503).send('server secret is not configured');
        }
