...
export async function createScript(name: string, cwd: string, content: string) {
  const { data } = await client.post<{ status: string; path: string; command: string }>('/scripts/create', { name, cwd, content })
  return data
}
