...
function resolveEvent(eventHeader, body) {
    ...
    if (eventHeader === 'merge_request_hooks') {
        ...
        const branch = body?.pull_request?.target_branch || null;
...
export function createWebhookHandler({ config, logger, runner }) {
    return (req, res) => {
        ...
        const eventHeader = req.get('X-Gitee-Event');
        const repo = extractRepo(body);
        const event = resolveEvent(eventHeader, body);

        if (!event || !repo) {
            logger.info('ignored event', { reason: 'unsupported', event: eventHeader, repo });
